...
public interface IssuedBookRepository extends JpaRepository<IssuedBook, Long> {
    ...
    @Query(
            value = "SELECT * FROM issued_book WHERE " +
                    ...
                    " (-1000 IN (:bookIds) OR id IN (:bookIds)) AND " +
                    " (-1000 IN (:studentIds) OR id IN (:studentIds)) AND " +
                    " (-1000 IN (:issuerIds) OR id IN (:issuerIds)) AND " +
                    " (-1000 IN (:receiverIds) OR id IN (:receiverIds))",
            nativeQuery = true
    )
